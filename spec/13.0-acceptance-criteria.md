...
все открытые» processes only records with an "active" status (§3.2 — `OPEN`, `HR_INTERVIEW`,
`TECH_INTERVIEW`), shows progress and a correct summary; one failure does not abort the rest. 16. No hh.ru request without `User-Agent`; on 429 a backoff retry, then
