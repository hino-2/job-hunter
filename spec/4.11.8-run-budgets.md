...
`VACANCY_SCAN_MAX_DETAILS` is reserved in a **synchronous planning pass** over a page's title-matched
candidates (`planPageWork`, §4.11.4), before the concurrent detail pool starts — every admission happens
in one uninterrupted synchronous loop, so the counter can never be read stale by a concurrent worker, and
an overshoot of the budget is impossible by construction. A slot reserved there and then skipped by a
worker's own stop/deadline check (§4.11.12) is a harmless over-count of the upper bound only: both
conditions terminate the whole run, so the budget is never consulted again afterwards.
