...
| `VACANCY_LEADS_LIST_LIMIT`             | no   | `3000`                                     | Max records in the GET /api/vacancy-leads response (§5.7)                                                     |
...
| `VACANCY_AI_BATCH_SIZE`                | no   | `20`                                       | Titles per stage-1 request (§4.12). Descriptions go one at a time. Range 1…30 (§4.12.3, measured on qwen3:4b-instruct) |
| `VACANCY_AI_CONCURRENCY`               | no   | `3`                                        | Concurrent AI calls — title batches and description verdicts (§4.11.4). Must equal `OLLAMA_NUM_PARALLEL` (§4.12.4). Range 1…10 |
| `VACANCY_AI_TIMEOUT_MS`                | no   | `30000`                                    | Model request timeout; on failure keywords decide (§4.12.3). Default assumes a GPU — raise it on a CPU-only Ollama (§4.12.4) |
