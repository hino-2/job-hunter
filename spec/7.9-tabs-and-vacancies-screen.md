...
Four tabs switched by MUI `Tabs` («Отклики», «HR-собес», «Тех-собес», «Вакансии») under `AppHeader`, above the active tab's content.
...
- **«Отклики», «HR-собес» and «Тех-собес» all render the same `ApplicationsScreen`**, mounted with `key={tab}` so that switching between
  them remounts the screen instead of reusing its local filter state. «HR-собес»/«Тех-собес» are presets, not separate screens: status
  filter fixed to `HR_INTERVIEW`/`TECH_INTERVIEW` respectively, sort fixed to that tab's interview date field (`hrInterviewAt` /
  `techInterviewAt`) in ascending order, so the nearest upcoming interview is listed first. «Сбросить фильтры» on these tabs returns to
  that same preset, not to the «Отклики» defaults.
- The inactive tab **unmounts**; unsaved edits are sent on `blur` (§7.3) first. The «Открытых: N / M» counter stays in `AppHeader` on all
  four tabs and refers to applications.
