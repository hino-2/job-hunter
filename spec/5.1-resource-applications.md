...
| --------- | ---------------------------------------------------------------- | ----------- | -------------------------------------------------------------- |
| `status`  | `ApplicationStatus`                                              | —           | Filter by status; `OPEN` matches every "active" status (§3.2 — `OPEN`, `HR_INTERVIEW`, `TECH_INTERVIEW`). Any active-status filter (`OPEN`, `HR_INTERVIEW`, `TECH_INTERVIEW`) additionally requires an open `result` (`IN_PROGRESS` or `OFFER`, §3.3), so the interview filters list only records still in play. `CLOSED` also matches a closed `result` regardless of `status`; absent — all |
