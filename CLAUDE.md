...
Порядок разработки (§14) идёт шагами; шаги 1–10 сделаны, следующий — **шаг 11: финальный
проход** (прогон критериев приёмки §13, README с инструкцией запуска). §12 перечисляет то,
что делать **нельзя** (cron, пагинация, тёмная тема, JWT, экспорт и т. д.) — не добавляй это
по своей инициативе.
