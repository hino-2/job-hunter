...
/**
 * Дефолтные значения настроек — ручная копия сидинга и последующих правок промптов
 * из миграций backend/src/database/migrations/1786700000001-CreateVacancySearchSettingsTable.ts
 * (ключевые слова и слова-исключения), 1786800000000-SharpenVacancyTitlePrompt.ts (промпт
 * названия) и 1787500000000-BlockDelimitVacancyDescriptionPrompt.ts (промпт описания).
 * Текст промптов — verbatim, менять только вместе с соответствующей миграцией: кнопка
 * «Вернуть промпт по умолчанию» (§7.9.4) пишет этот текст напрямую в БД, и рассинхрон
 * с миграцией навсегда оторвал бы такую запись от её же guarded UPDATE в будущем.
 */
