...
`COMPANY_LOGO_DIR` (дефолт `os.tmpdir()/job-hunter-logos`) с таймаутом
`COMPANY_LOGO_REQUEST_TIMEOUT_MS` (дефолт `5000`).
...
Каталог логотипов компаний (`COMPANY_LOGO_DIR`) эфемерный — под него намеренно не
заведён docker-volume. После `docker compose down`/пересоздания контейнера `api`
файлы логотипов пропадают (запись в БД остаётся), шапка временно показывает
...
Прочие переменные поиска (шаблон ссылки на выдачу hh.ru, бюджеты прогона, режимы
отбора) — в [`.env.example`](./.env.example), раздел «Поиск, отбор и отображение
вакансий с hh.ru». `HH_SEARCH_URL_TEMPLATE` обязан содержать оба плейсхолдера
`{text}` и `{page}` — их отсутствие роняет старт приложения с понятным сообщением.
