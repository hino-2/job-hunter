...
Персональный трекер откликов на вакансии с автообновлением статуса по публичной странице
вакансии hh.ru. Запускается локально в Docker, пользователь один.
...
- автоподстановка компании и должности по вставленной ссылке на вакансию hh.ru;
- ручное обновление статуса — по одной записи (🔄) и разом по всем открытым;
...
- «+ Добавить» → вставь ссылку на вакансию hh.ru — компания и должность подставятся сами,
...
- 🔄 в шапке записи — обновить статус одной вакансии по данным hh.ru.
...
браузер → web (nginx :8080) → api (NestJS :3000) → db (PostgreSQL :5432)
                ↓                      ↓
          статика React        страница вакансии hh.ru
```
...
`hh-sync.service`, e2e синхронизации (`POST /:id/sync`, `POST /sync-open`) и фронтовые
компонентные тесты — см. §13.20 и отметки к шагам 6–10 в §14 спецификации.
...
- **Синхронизация возвращает ошибку на всех записях.** Проверь `HH_USER_AGENT` в `.env`
  и доступность `hh.ru` с машины, где запущен контейнер `api`.
- **Синхронизация массово отдаёт `403`.** hh.ru заблокировал запросы с этого User-Agent
