...
хост из allow-list hh.ru, оба плейсхолдера `{text}` и `{page}`). Кто раньше задавал
`HH_SEARCH_URL_TEMPLATE` в `.env` — после обновления перенесите то же значение в диалог
настроек один раз, старую переменную можно удалить.
