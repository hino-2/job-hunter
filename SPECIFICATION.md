...
1. Записать `vacancy_archived`, `last_synced_at`, `OK`, `last_sync_error = null`.
...
15. ~~**Интеграция с getmatch.ru и обобщение до источников вакансий.**~~
    ...
    **Миграция** `GeneralizeVacancySource` переименовывает `hh_vacancy_id` →
    ...
    отдельной обработки «неопознанных» записей не требуется. `down()` полностью обратима,
    включая восстановление `hh_vacancy_type` (данных не теряем — колонка всегда была `null`).
    ...
    **Тестовое покрытие — только правки существующих файлов**, новых spec-файлов нет
    ...
    портах — по одному на источник) с добавленными getmatch-кейсами (активная, `is_active:
    false`, `initialVacancy:null` → 404, payload без ключа → 502, `429` → 502 после ретраев,
    путь и заголовки запроса). Новый `test/getmatch.fixtures.ts` — тест-инфраструктура,
