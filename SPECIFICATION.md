...
**Откуда берётся URL.** Каждый парсер страницы источника (§4.2) вытаскивает `src`
логотипа из своего блока разметки:

| Источник     | Разметка                                                   |
| ------------ | ----------------------------------------------------------- |
| hh.ru        | `<img src="…">` внутри `div[data-qa="vacancy-company-logo"]` |
| getmatch.ru  | `<img src="…">` внутри `div.b-company-logotype`             |

Найденный `src` немедленно абсолютизируется относительно `*_SITE_BASE_URL` источника
...
> **Наблюдение по hh.ru.** На момент реализации статическая HTML-страница вакансии
> hh.ru не содержит атрибута `src` у `<img>` внутри `data-qa="vacancy-company-logo"`
> ни для одной проверенной вакансии (в т. ч. у крупных работодателей) — логотип
> подгружается клиентским JS уже после серверного рендера. Это означает, что при
> текущей вёрстке `hasCompanyLogo` у hh.ru-записей практически всегда остаётся
> `false`: парсер и allow-list реализованы по документированной здесь схеме и рабочие,
> просто исходных данных для неё в статическом HTML сейчас нет. У getmatch.ru та же
> схема работает полностью — страница отдаёт относительный `src`
> (`/uploads/companies_logos/{uuid}.png`), который абсолютизируется и скачивается
> без проблем. Если вёрстка hh.ru изменится — исправление ограничится
> `HH_COMPANY_LOGO_PATTERN`, остальной конвейер не трогает.
...
| --- | --------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Компания        | `flex: 0 0 220px`, `noWrap` + `Tooltip` при обрезке | `Avatar` 24px (логотип компании либо буква-фолбэк, §4.10) + `Typography` bold                                                                                                                                                        |
...
20. **Логотип компании в свёрнутой шапке записи.** ✅ **Сделано.** Новая
    ...
    парсеры страниц (`div[data-qa="vacancy-company-logo"] img[src]` у hh.ru,
    ...
    реальных hh.ru/getmatch.ru (§4.10): у getmatch.ru конвейер работает end-to-end
    (скачивание, атомарная запись, self-healing после удаления файла, отдача байт
    с верным `Content-Type`); у hh.ru на момент реализации статическая страница не
    отдаёт `src` в блоке логотипа (подгружается клиентским JS), поэтому
    `hasCompanyLogo` у hh.ru-записей практически всегда `false` — подробности и
    единственная точка правки при смене вёрстки hh.ru см. §4.10. Пара
