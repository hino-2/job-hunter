...
Персональный трекер отправленных отзывов на вакансии. Пользователь ведёт список вакансий, на которые он откликнулся, фиксирует по каждой контакты, даты собеседований, заметки и итоговый результат. Статус вакансий (открыта / снята с публикации) подтягивается автоматически с публичной страницы вакансии hh.ru.
...
| HTTP-клиент    | `@nestjs/axios` 4                                 | загрузка HTML-страницы вакансии hh.ru                               |
...
7. **Страница вакансии разбирается регексом + `JSON.parse`, без HTML-библиотеки.**
   Нужны ровно две лексические операции (тело `<script type="application/ld+json">` —
...
| `hh_vacancy_type`   | `varchar(32)`  | нет   | `null`              | **Не заполняется:** страница вакансии не содержит `type.id` (§4.1). Колонка сохранена, синхронизация пишет в неё `null` |
...
Парсер извлекает из ссылки только `vacancy_id` — хост самого запроса к hh.ru всегда берётся
из `HH_SITE_BASE_URL`, а не из региона в исходном URL пользователя (§4.1).

### 4.3 Правила применения результата синхронизации

При успешном получении вакансии:

1. Записать `hh_archived`, `hh_vacancy_type = null`, `last_synced_at`, `OK`, `last_sync_error = null`.
...
> (§4.1) — страница вакансии его не содержит, признак закрытия теперь только `archived`.
...
В ответе preview поле `vacancyType` всегда `null` (§4.1 — страница не содержит `type.id`); автоподстановка использует только `company` и `position`.
...
| `ERROR`          | Сетевая ошибка, таймаут, 5xx, `403`, нераспознанная страница (нет признака архивности) | Ошибка обновления           |

### 4.6 Надёжность запросов к hh.ru

- Таймаут запроса: **10 000 мс**.
- Ретраи: до **2** повторов на 429 и 5xx, экспоненциальный backoff (500 мс, 1500 мс). На 4xx кроме 429 (включая `403`) — без ретраев.
...
- Ответ ограничен по размеру (`HH_MAX_RESPONSE_BYTES`, 4 МиБ): страница вакансии — около
  164 КБ по проводу с gzip и около 772 КБ несжатых, лимит — пятикратный запас.
...
Поле `vacancyType` сохранено в контракте, но всегда `null` (§4.1 — страница вакансии
не содержит `type.id`).
...
- Сетевая ошибка / таймаут / 5xx / `403` / нераспознанная страница → `502` с телом ошибки.
...
│  │                             ✅ hh.fixtures.ts (строит минимальную HTML-страницу
│  │                             ✅ вакансии), hh-preview.e2e-spec.ts
│  └─ src/
...
│        ├─ hh-api.service.ts            ✅ HTTP-клиент (страница вакансии), ретраи,
│        │                               ✅ таймауты (+ .spec.ts)
...
- Эмуляция браузера, headless-браузер, обход анти-бот проверок и ddos-guard, cookie-сессии
  hh.ru; извлечение со страницы вакансии чего-либо, кроме JSON-LD `JobPosting` и признака
  архивности (описание, зарплата, навыки, требования); хранение HTML-дампов страниц.
...
После шагов 4, 6, 8, 10 и 12 — обязательный проход `code-reviewer`.
