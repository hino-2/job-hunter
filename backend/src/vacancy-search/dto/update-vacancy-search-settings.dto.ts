 ...
 *
 * searchUrlTemplate — обычное поле настроек (§3.6), больше не значение env, и
 * единственный источник поискового запроса: пользователь вставляет уже готовую
 * ссылку выдачи hh.ru со своим text=…. Плейсхолдер {page} проверяется тем же
 * паттерном, что подставляет buildHhSearchUrl (hh/hh.constants.ts), а происхождение
 * (https + хост hh.ru, SSRF-защита) — отдельным ValidatorConstraint
 * (SearchUrlTemplateConstraint), потому что @Matches не умеет разбирать URL.
 *
...
export class UpdateVacancySearchSettingsDto {
  ...
  /**
   * §3.6/§4.11.1/§5.7: шаблон ссылки на выдачу hh.ru. Плейсхолдер {page} проверяется
