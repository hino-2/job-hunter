 ...
 * и applications.service (§4.2 — hh_vacancy_id вычисляется при каждой записи
 * vacancy_url). Оформление парсера провайдером Nest потребовало бы, чтобы
 * ApplicationsModule импортировал HhModule, а HhModule в шаге 6 импортирует
 * ApplicationsModule ради hh-sync.service — то есть циклическая зависимость модулей
 * и forwardRef на ровном месте. Зависимостей у парсера нет, DI ему ничего не даёт.
 *
